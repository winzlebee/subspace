<script lang="ts">
  import { servers, currentServerId, showCreateServer, showSettings, currentUser, logout as doLogout } from "$lib/stores";
...
<div class="flex flex-col items-center w-[72px] bg-base-300 py-3 gap-2 shrink-0 overflow-y-auto">
  <!-- Home / DM button -->
  <button
  ...
  {#each $servers as server (server.id)}
    <div class="indicator">
      {#if server.id === $currentServerId}
        <span class="absolute -left-1 top-1/2 -translate-y-1/2 w-1 h-8 bg-base-content rounded-r-full"></span>
      {/if}
      <button
        ...
        title={server.name}
        on:click={() => onSelectServer(server.id)}
      >
        {#if server.icon_url}
          <img src={server.icon_url} alt={server.name} class="w-full h-full rounded-[inherit] object-cover" />
        {:else}
  ...
  <button
    ...
    title="Add a Server"
    on:click={() => showCreateServer.set(true)}
  >
    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  >
    {#if $currentUser?.avatar_url}
      <img src={$currentUser.avatar_url} alt="avatar" class="w-full h-full rounded-full object-cover" />
    {:else if $currentUser}
